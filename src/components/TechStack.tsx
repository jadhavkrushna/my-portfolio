...
// Shared ref for active state — avoids React re-renders on scroll
...
const TechStack = () => {
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (workEl) {
          ...
          isActiveRef.current = scrollY > threshold;
        }
        ticking = false;
      });
    };

    // Debounced nav click handler
    ...
    return () => {
      ...
      navLinks.forEach((elem, i) => {
        elem.removeEventListener("click", clickHandlers[i]);
      });
    };
  }, []);
  ...
  // Pre-compute material assignments so they don't change on re-render
